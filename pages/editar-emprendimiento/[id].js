...
import CategorySelector from '../../components/emprendimientos/CategorySelector';
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      try {
        ...
        if (!found) {
          setMessage('No tienes acceso a este emprendimiento');
          setOpen(true);
          setLoading(false);
          return;
        }
        ...
        setInstagramProfile(found?.instagramProfile || '');
        setShowContact(Boolean(found?.showContact));
        if (Array.isArray(found?.categories)) {
          setCategoryIds(found.categories.map((c) => c.id));
        }
        if (Array.isArray(found?.photos)) {
          const existingIds = found.photos.map(p => p.id).filter(Boolean);
...
export default function EditarEmprendimientoByIdPage() {
  return (
    <AuthContextProvider>
      <EditProjectContent />
    </AuthContextProvider>
