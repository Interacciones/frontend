  ...
  return (
    <div className="md:w-1/2 lg:w-1/3 m-2 py-3 px-5 bg-white rounded-md shadow-md">
        <div className='flex place-items-center px-3'>
            ...
            <h1 className='font-bold ml-1'>{roundToTwo(average)} <span className='font-normal text-gray-500'>({reviewAmount} reseñas)</span></h1>
        </div>
        <div className='p-0'>
                ...
                <div key={starRating} className="bg-gray-200 rounded-md h-6 my-2 p-0 overflow-hidden relative group">
                    <div style={{ width: `${starPercentages[5 - starRating]?.percentage}%` }} className={`h-6 bg-yellow-400 place-items-center px-0 align-middle`}>
                        <div className='flex w-screen place-items-center h-full mx-2 items-center align-middle'>
                            ...
                            <h1 className='font-semibold ml-1 text-sm'>{starsCount[5 - starRating]} <span className='font-normal'>({starPercentages[5 - starRating]?.percentage}%)</span></h1>
                        </div>
                    </div>
