...
export default function ReportCommentModal({ onClose, commentId }) {
  const { user } = UserAuth();
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('https://interserver.lat/reports/project-comment', {
        method: 'POST',
        headers: {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-75">
        <div className="bg-white w-[95%] md:w-3/4 lg:w-[60%] rounded-md p-4 shadow-lg mx-auto text-center">
          <p className="text-black text-lg font-bold mb-2">
            Para poder reportar comentarios, necesitas verificar tu usuario
          </p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-75">
      <div className="bg-white w-[95%] md:w-3/4 lg:w-[60%] rounded-md p-4 shadow-lg">
        <h2 className="text-xl font-semibold text-gray-900 mb-3">Reportar comentario</h2>
